...
int dram_init(void)
{
	...
	for (i = 0; i < len; i++) {
		...
		bank_start = of_read_number(prop, na);
		prop += na;
		bank_size = of_read_number(prop, ns);
		prop += ns;
		...
		tegra_mem_map[ram_bank_count].base = bank_start;
	...
	tegra_mem_map[ram_bank_count + 1].base = 0;
...
void dram_init_banksize(void)
{
	...
	for (i = 0; i < ram_bank_count; i++) {
		gd->bd->bi_dram[i].start = tegra_mem_map[1 + i].base;
